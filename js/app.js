...
async function fromOptions() {
  ...
  async function fetchTanggalMerah() {
    // Example implementation of fetching red dates
    try {
      const response = await fetch('/api/tanggal-merah'); // Replace with your API endpoint
      return response.json(); // Assuming the response is JSON
    } catch (error) {
      console.error('Error fetching tanggal merah:', error);
      return [];
    }
  }

  function isTanggalMerah(date, tanggalMerah) {
    // Check if the given date is in the list of red dates
    return tanggalMerah.some(d => new Date(d).toDateString() === date.toDateString());
  }
  ...
  function syncSelectOptions(sourceId, targetId) {
    ...
    if (sourceSelect && targetSelect) {
      targetSelect.innerHTML = sourceSelect.innerHTML; // Copy options
      targetSelect.disabled = sourceSelect.disabled; // Sync disabled state
    }
  }

  function saveToLocalStorage() {
    const frommod1 = document.getElementById('frommod1');
    const tomod2 = document.getElementById('tomod2');

    // Save the selected value and options of frommod1
    localStorage.setItem('fromDate', from1Date);
    localStorage.setItem('frommod1Value', frommod1.value);
    localStorage.setItem('frommod1Options', JSON.stringify(Array.from(frommod1.options).map(option => option.value)));
    
    // Save the selected value and options of tomod2
    localStorage.setItem('toDate', document.getElementById('to2').value);
    localStorage.setItem('tomod2Value', tomod2.value);
    localStorage.setItem('tomod2Options', JSON.stringify(Array.from(tomod2.options).map(option => option.value)));
  }
